...
class FloatingBubbleService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // stopService() can never trigger ForegroundServiceDidNotStartInTimeException.
        try {
            startForeground(NOTIFICATION_ID, buildNotification())
        } catch (e: Exception) {
    ...
    private class BubbleView(context: Context, private var count: Int, private val sizePx: Int) :
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            // Always draw a border — alert color when count is high, normal
            // otherwise. Inset by half the stroke so the outer edge of the stroke
            // isn't clipped by the view bounds.
