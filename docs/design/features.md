...
| Parking reminder | Removed | Extracted into a separate standalone app, ParkingValet. See that repo for ongoing development. |
...
| expo-share-intent migration | Built · QA pending | Spawned by SDK-57 upgrade. Replace custom `ShareIntentModule.kt` + JS bridge with `expo-share-intent` (v6+). **Android-only** this pass (iOS share target deferred to its own story). Keep `shareText.ts` parser + `useShareIntent() {prefill,clearPrefill}` interface + all 16 Share-text criteria. Docs: `docs/design/features/expo-share-intent-migration/` |
...
| Harden native-package registration scripts (fail-loud) | Idea | Build-pipeline hardening. `scripts/copy-native-files.js` + `patch-native-config.js` register the custom `FloatingBubblePackage` by regex against the Expo template's `// Packages that cannot be autolinked` anchor. If a future SDK renames that anchor, the insert becomes a silent no-op (`content !== before` false → no write, **no error**) and the package is dropped at runtime with no build failure. Fix: after the registration block, assert the `add(FloatingBubblePackage())` string is present in the written file and `throw`/warn if absent, so template drift fails loud. Surfaced during the expo-share-intent migration review (pre-existing). |
| | | |
