...
| Upgrade Expo SDK 54 → 57 | Built · QA pending | Enabler (spawned by Share-text analysis) — unlocks `expo-share-intent` + iOS share target; newer RN & security patches. Landed **Expo SDK 57 / RN 0.86.2 / React 19.2** via 3 incremental hops on `develop` (cb6430a 54→55, ab43fbd 55→56, 59e9a6c 56→57). Automated verification green (147 tests, doctor 20/20, native reconciliation + idempotency); AC3 release-signing + AC5 device behavior pending on-device QA (no JDK/SDK in agent env). Docs: `docs/design/features/upgrade-expo-sdk-57/` |
| expo-share-intent migration + iOS share target | Idea | Spawned by SDK-57 upgrade. Replace custom `ShareIntentModule.kt` + `src/modules/ShareIntent.ts` + `useShareIntent.ts` with `expo-share-intent` (v6+, needs SDK 55+); adds iOS share parity. Unblocked once SDK upgrade lands. |
