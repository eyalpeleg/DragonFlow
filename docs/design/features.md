...
| Upgrade Expo SDK 54 → 57 | Building | Enabler (spawned by Share-text analysis) — unlocks `expo-share-intent` (replaces custom-native share code) + iOS share target; also newer RN & security patches. High blast radius; schedule as its own effort. Scope: pure incremental upgrade 54→55→56→57 (share-intent migration deferred to its own story). Docs: `docs/design/features/upgrade-expo-sdk-57/` |
