...
| Harden native-package registration scripts (fail-loud) | Idea | Build-pipeline hardening. `scripts/copy-native-files.js` + `patch-native-config.js` register custom packages (FloatingBubble, ParkingWatcher) by regex against the Expo template's `// Packages that cannot be autolinked` anchor. If a future SDK renames that anchor, the insert becomes a silent no-op (`content !== before` false → no write, **no error**) and the package is dropped at runtime with no build failure. Fix: after each registration block, assert the `add(XPackage())` string is present in the written file and `throw`/warn if absent, so template drift fails loud. Surfaced during the expo-share-intent migration review (pre-existing). |
| | | |
